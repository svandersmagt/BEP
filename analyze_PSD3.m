function [MLfit, Fmag, Rfit] = analyze_PSD3(fs,Ext,y,R,kT,eta)
...
[f, PSD, ~] = calc_powerspblock(y,fs);
f(1) = []; PSD(1) = [];
goodinds = f > f_c(1)/20;

%%% Max likelihood fit to PSD (using goodinds)
%%% According to the model by Lansdorp and Saleh (RSI 2012)
[par, ~, ~] = fminsearch(@(par) costfunction_PSD3fit2(par(1),fs,f(goodinds),L,par(2),PSD(goodinds),kT,eta), [F_est R_est]); %(Fmag,fs,f,L,R,PSD)
Fmag = par(1); Rfit = par(2); MLfit = [Fmag Rfit fitgood];
PSDmodel = analytical_PSD3_overdamped_bead(Fmag,fs,f,L,Rfit,kT,eta);
