function [MLfit, Fmag, Rfit] = analyze_PSD3(fs,mean_z,y,R)
...
[par, ~, ~] = fminsearch(@(par) costfunction_PSD3fit(par(1),fs,f(goodinds),L,par(2),PSD(goodinds)), [F_est R_est]); %(Fmag,fs,f,L,R,PSD)
Fmag = par(1); Rfit = par(2); MLfit = [Fmag Rfit fitgood];
PSDmodel = analytical_PSD3_overdamped_bead(Fmag,fs,f,L,Rfit);
